...
function createImg(src, alt, width) {
  width = width || false;
  let imgUrl = `<img src="${src.trim()}" alt="${alt.trim()}"`;
  if (width !== false) {
    if (typeof width === 'string' && width.match(/^[0-9]+$/)) {
      let intWidth = parseInt(width);
      return `${imgUrl} width="${intWidth}">`;
    }
    if (typeof width === 'number') return `${imgUrl} width="${width}">`;
  }
  return `${imgUrl}>`;
}
...
function parseDateString(value) {
  if (value) {
    if (value.match(/\d{4}-\d{2}-\d{2}$/g)) {
      ...
      dateObject = new Date(dateString[0], dateString[2] - 1, dateString[1]);
      return dateObject;
    }
    if (value.match(/[a-zA-Z]+/g)) throw 'This is a string, but not a date string';
    if (value.match(/\d{1,3}-\d{2}-\d{2}$/g))
      throw 'Invalid year. You need to type four digits -> YYYY';
    if (value.match(/\d{4}-\d{1}-\d{2}$/g)) throw 'Invalid day. You need to type two digits -> DD';
    if (value.match(/\d{4}-\d{2}-\d{1}$/g))
      throw 'Invalid month. You need to type two digits -> MM';
    if (value.match(/\s+/g)) throw 'Invalid separators. It is spaces vs dashes';
    if (value.match(/\/+/g)) throw 'Invalid separators. It is slashes vs dashes';
  }
...
function toDateString(value) {
  ...
  let monthString = value.getMonth() + 1;
  let dateString = value.getDate();
  return `${yearString}-${dateString < 10 ? '0' : ''}${dateString}-${
    monthString < 10 ? '0' : ''
  }${monthString}`;
}
...
function formatCoord(lat, lng, includeBrackets) {
  includeBrackets = includeBrackets || false;
  if (!validateCoord(lat, lng)) throw 'Invalid coordinates';
  return `${includeBrackets ? '[' : ''}${lat},${lng}${includeBrackets ? ']' : ''}`;
}
...
function formatCoords(...values) {
  let evenNumber = arguments.length % 2;
  let isValid = true;
  let outputString;
  if (evenNumber !== 0) throw 'lat & lng must be in pairs';
  for (let i = 0; i <= arguments.length - 2; i++, i++)
    isValid = validateCoord(values[i], values[i + 1]);
  if (!isValid) throw 'lat & lng must be decimal numbers';
  outputString = '[';
  for (let i = 0; i <= arguments.length - 2; i++, i++)
    outputString =
      outputString + (i === 0 ? '' : ', ') + formatCoord(values[i], values[i + 1], true);
...
function mimeFromFilename(filename) {
  let croppedString = filename.replace(/.*\./, '');
  switch (croppedString) {
    case 'html':
    case 'htm':
      return 'text/html';
    case 'css':
...
function generateLicenseLink(licenseCode) {
  let stringCC;
  let licenseURL = !licenseCode ? '' : licenseCode.replace(/CC-/, '');
  switch (licenseURL) {
    case 'BY':
      stringCC = 'Creative Commons Attribution License';
      break;
    case 'BY-NC':
      stringCC = 'Creative Commons Attribution-NonCommercial License';
      break;
    case 'BY-SA':
      stringCC = 'Creative Commons Attribution-ShareAlike License';
      break;
    case 'BY-ND':
      stringCC = 'Creative Commons Attribution-NoDerivs License';
      break;
    case 'BY-NC-SA':
      stringCC = 'Creative Commons Attribution-NonCommercial-ShareAlike License';
      break;
    case 'BY-NC-ND':
      stringCC = 'Creative Commons Attribution-NonCommercial-NoDerivs License';
      break;
    default:
      stringCC = 'All Rights Reserved';
      return `<a href="https://choosealicense.com/no-permission/">${stringCC}</a>`;
  }
  return `<a href="https://creativecommons.org/licenses/${licenseURL.toLowerCase()}/4.0/">${stringCC}</a>`;
}
...
function pureBool(value) {
  ...
  if (typeof value === 'number') {
    if (value === 0 || value === -1 || isNaN(value)) return false;
    else if (value === 1) return true;
    throw 'invalid boolean value';
  } else if (typeof value === 'string') {
    stringLowerCase = value.toLowerCase();
    if (
...
function all() {
  let result = true;
  for (let i = 0; i < arguments.length && result === true; i++) {
    try {
      result = pureBool(arguments[i]);
    } catch (error) {
      return false;
    }
  }
...
function none() {
  let result = false;
  let i;
  for (i = 0; i < arguments.length && result === false; i++) {
    try {
      result = pureBool(arguments[i]);
    } catch (error) {
      return false;
...
function buildUrl(q, perPage, order, license, lat, lng) {
  ...
  if (perPage < 1 || perPage > 200) throw 'Invalid range';
  if (!(order === 'asc' || order === 'desc')) throw 'Invalid order';
  ...
  if (!validateCoord(lat, lng)) throw 'Invalid coordenates';
  return `https://api.inaturalist.org/v1/observations?q=${query}&per_page=${perPage}&order=${order}&license=${license}&lat=${lat}&lng=${lng}`;
}
